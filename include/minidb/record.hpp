...
namespace minidb
{
    ...
    // On-disk layout of a record (6 bytes, little-endian):
    ...
    struct record
    {
        ...
        [[nodiscard]] static optional<record> deserialize(
            const std::array<uint8_t, SIZE> &bytes) noexcept
        {
            ...
            return record{ts, bytes[DEVICE_ID_OFFSET]};
        }
    };
}
